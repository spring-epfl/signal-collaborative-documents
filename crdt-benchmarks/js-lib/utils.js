...
/**
 * Serialize an id and a binary update into a base64 string.
 * @param {number} id
 * @param {Uint8Array|Buffer} update
 * @returns {string} Base64-encoded payload
 */
export function serializeMessage(id, update) {
  const idBuf = Buffer.allocUnsafe(4);
  idBuf.writeUInt32BE(id, 0);
  const dataBuf = Buffer.from(update);
  return Buffer.concat([idBuf, dataBuf]).toString('base64');
}

/**
 * Deserialize a base64-encoded message back into id and update.
 * @param {string} b64str
 * @returns {{id: number, update: Buffer}}
 */
export function deserializeMessage(b64str) {
  const buf = Buffer.from(b64str, 'base64');
  const id = buf.readUInt32BE(0);
  const update = buf.slice(4);
  return { id, update };
}
