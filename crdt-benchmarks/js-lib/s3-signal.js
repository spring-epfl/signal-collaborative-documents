...
if (!isMainThread) {
  ...
  console.log('[B3][WORKER] Setting up SSE listener');
  ...
  es.addEventListener('receive', async e => {
    ...
    if (info.type === 'DELIVER') {
      ...
      console.log(`[B3][WORKER] Sending ${id}, ${receivingTimestamp}, ${serverReceivedTimestamp}, ${serverDeliveredTimestamp}, ${envelopeTimestamp} to parent thread`);
...
export const runBenchmarkB3Signal = async (crdtFactory, filter) => {
  if (isMainThread) {
    console.log('[B3][MAIN] Spawning worker for SSE listener');
    const worker = new Worker(new URL(import.meta.url), { type: 'module' });
    // Capture signal metrics forwarded from the worker thread
    worker.on('message', msg => {
      console.log(`[B3][MAIN] Received from worker: `, JSON.stringify(msg));
    ...
    await new Promise((resolve, reject) => {
      const onReadyMessage = (msg) => {
        if (msg === 'ok') {
          console.log('[B3][MAIN] Worker ready, starting benchmarks');
          cleanup();
          resolve();
...
export const runBenchmarkB3 = async (crdtFactory, filter) => {
  ...
  const benchmarkTemplate = async (id, inputOps1, inputOps2, check) => {
    ...
    const doc1 = crdtFactory.create(update => mux(async () => {
      ...
      console.log(`[B3][USER1:${account1}] processing update #${id} (${update.length} bytes)`);
      const serializationTime = 0; // already serialized
      console.log(`[B3][USER1:${account1}] Sending message #${id}`);
    ...
    const doc2 = crdtFactory.create(update => mux(async () => {
      ...
      updateIdCounter++;
      const id = updateIdCounter;
      console.log(`[B3][USER2:${account2}] processing update #${id} (${update.length} bytes)`);
      ...
      console.log(`[B3][USER2:${account2}] Sending message #${id}`);
    ...
    while (signalMetrics.size < expected) {
      console.log(`[B3][${id}] Waiting for ${expected - signalMetrics.size} Signal events...`);
  ...
  const benchmarkName = '[B3-Signal] Concurrent overlapping edits';
  ...
  });
  {
    ...
    writeFileSync(new URL('../../benchmark_data/s3-signal.csv', import.meta.url), rows.join('\n'));
  }
}
