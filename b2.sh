...
log "Starting Automerge b2 benchmark run"
...
signal-cli --config="$SIGNAL_CONFIG" --verbose daemon --http > signal-cli.2.log 2>&1 &
SIGNAL_PID=$!
log "signal-cli started in background (pid=$SIGNAL_PID). Waiting ${SLEEP_SECS}s for readiness..."
sleep "$SLEEP_SECS"

# Run benchmarks
log "Running Automerge b2-signal benchmark..."
(
